...
import { getWeatherApiData } from '../services/weatherApiService';
...
const WeatherApiWidget = () => {
  ...
  const fetchWeatherData = async () => {
    ...
    try {
      ...
      console.log('Fetching WeatherAPI data for location:', { latitude, longitude });
      const weatherData = await getWeatherApiData(latitude, longitude);
      console.log('WeatherAPI data received:', weatherData);
      setWeather(weatherData);
      setLastUpdated(new Date());
    } catch (error) {
      console.error('Error fetching WeatherAPI weather:', error);
    ...
    return (
      <div className="bg-gradient-to-br from-red-400 via-red-500 to-red-600 rounded-2xl p-6 text-white shadow-xl">
        <div className="text-center">
          <div className="text-xl font-bold mb-2">WeatherAPI Unavailable</div>
        ...
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-start space-x-1 mt-1">
            ...
            <div>
              ...
              <div className="text-xs opacity-60 mt-1">WeatherAPI</div>
            </div>
          </div>
          <button
            onClick={handleRefresh}
